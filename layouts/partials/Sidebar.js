...
import { FaCheck, FaRegCalendar, FaUserAlt } from "react-icons/fa";
...
const Sidebar = ({ posts, categories, className }) => {
  ...
  return (
    <aside className={`${className} px-0 lg:px-6 lg:col-4`}>
      {about.enable && (
        <div className="relative rounded border border-border p-6 text-center dark:border-darkmode-border bg-theme-light dark:bg-darkmode-theme-light font-semibold">
          <ImageFallback
            className="-z-[1]"
            src="/images/map.svg"
            fill={true}
      ...
      {featured_posts.enable && (
        <div className="mt-6 rounded border border-border p-6 dark:border-darkmode-border bg-theme-light dark:bg-darkmode-theme-light">
          <h4 className="section-title mb-12 text-center">Destaques</h4>
          <div className="mb-12 flex items-center justify-center">
            <button
              className={`btn px-5 py-2 rounded-lg ${showRecent ? "btn-outline-primary" : "btn-primary"
                }`}
            ...
            <button
              className={`btn ml-3 px-5 py-2 rounded-lg ${showRecent ? "btn-primary" : "btn-outline-primary"
                }`}
      ...
      {/* newsletter */}
      {newsletter.enable && (
        <div className="mt-6  rounded border border-border p-6 text-center dark:border-darkmode-border bg-theme-light dark:bg-darkmode-theme-light">
          <h4 className="section-title">{newsletter.title}</h4>

          {markdownify(newsletter.content, "p", "mt-10")}

              <CustomForm />

          <p className="text-xs flex justify-start align-center">
            <FaCheck className="text-primary"/> Assinando, você concorda com a
            <Link
